...
func compareRank(firstRank: Rank, secondRank: Rank) -> String {
    if firstRank.rawValue > secondRank.rawValue {
        return "\(firstRank) is greater than \(secondRank)"
    }
    else if firstRank.rawValue < secondRank.rawValue {
        return "\(firstRank) is smaller than \(secondRank)"
    } else {
        return "Two Ranks are equal"
    }
}

compareRank(Rank.Five, secondRank: Rank.Queen)
compareRank(Rank.King, secondRank: Rank.Ace)
compareRank(Rank.Eight, secondRank: Rank.Eight)
//: > **Experiment**:
...
enum ServerResponse {
    ...
    case Error(String)
    case Indecisive(String)
}

let success = ServerResponse.Result("6:00 am", "8:09 pm")
let failure = ServerResponse.Error("Out of cheese.")
let indecisive = ServerResponse.Indecisive("Unable to decide the sunrise and sunset time!")
...
switch indecisive {
    ...
    case let .Error(error):
        print("Failure...  \(error)")
    case let .Indecisive(unable):
        print("\(unable). Please come back later.")
}
//: > **Experiment**:
